...
function clearInputTable() {
  ...
  for (i = 0; i < 3; i++) {
    document.getElementById("studentListTable").rows[1].cells[i].childNodes[0].value = "";
    document.getElementById("studentListTable").rows[1].cells[i].style.backgroundColor = "white";
  }
...
function sortClass(method) {
  ...
  for (j = 0; j < groupDetailsObject.numGroupsA; j++) {
    ...
    for (x = 0; x < groupDetailsObject.numStudentsA; x++) {
      smallGroupArray.push(
        sortedArray[x + (i * groupDetailsObject.numStudentsB + j * groupDetailsObject.numStudentsA)]
      );
    }
    groupedClassArray.push(smallGroupArray);
  }
...
function displaySavedClasses() {
  ...
  for (i = 0; i < allClassesArray.length; i++) {
    ...
    buttonElement0 = document.createElement("button");
    buttonElement0.innerHTML = "Delete Class";
    buttonElement0.id = "btnDelete" + cell0.innerHTML;
    buttonElement0.setAttribute(
      "onclick",
      "deleteSavedClass(this.id);" + onclick
    );
    buttonElement0.classList.add("btn", "btn-outline-secondary");
    cell1.appendChild(buttonElement0);
  }
...
function resetHome() {
  ...
  document.getElementById("showAllClassesBtn").style.display = "inline";
  document.getElementById("hideAllClassesBtn").style.display = "none";
  goToPage(page2);
}
